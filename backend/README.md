# Backend - BlareXSense Smart Switch Dashboard

## Overview

FastAPI backend for the BlareXSense ESP32 dashboard. It provides:
...
Most backend settings live in [config.json](config.json), so deployment values can be changed without editing Python code.
...
Stop the backend, move or delete `backend/data/BlareXSense.db`, then start the backend again. The schema is recreated automatically.
